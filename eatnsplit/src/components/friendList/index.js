...
const Freind=({setBillUI,userData,indxNum,setUserIndex})=>{
return(
    <div style={{display:'flex', alignItems:'center',gap:'60px', border:"1px solid black"}}>
        ...
        <div>
            <p>{userData.name}</p>
            <p>{userData.amount>0? `you owes ${userData.amount} Rupees `:userData.amount==0?`You and ${userData.name} are Even `:`${userData.name +" "+ Math.abs(userData.amount)  } Rupees  owes you`}</p>
        </div>
        <button onClick={()=>{
            setUserIndex(indxNum);
            
            setBillUI(true);}}>Select</button>
    </div>
)
}
...
export const FriendList = ({billUi,setBillUi,user,setUser,setUserIndex}) => {
  ...
  return (
    <div>
        {data.map((e,index)=>( <Freind setBillUI={setBillUi}setUserIndex={setUserIndex} indxNum={index} userData={e}/>))}
        
        
        <button onClick={()=>setIsVisible(!isVisible)}>{!isVisible ? 'Add Freind' :'close'}</button>
        {isVisible && (
        <div >
            
            <div style={{display:'flex', gap:'20px'}}>
                <p>FrendName:</p><input type='text'  onChange={(e)=>{
                    setUserName(e.target.value)
                }}/>
            </div>
            <div style={{display:'flex', gap:'20px'}}>
                <p>ImageUrl:</p><input type='text'onChange={(e)=>{
